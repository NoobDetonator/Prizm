#!/usr/bin/env node
/**
 * T0.2 — Prove which sliders change pixels (mean absolute difference).
 * Writes docs/slider-audit-before.md
 */
...
const outFile = path.join(root, 'docs', 'slider-audit-before.md')
...
async function main() {
  ...
  for (const id of sliders) {
    const result = await page.evaluate(async (sliderId) => {
      ...
      return { ok: true, min, max, mad, width: a.width, height: a.height }
    }, id)

    if (!result.ok) {
      rows.push({ slider: id, changed: false, note: result.reason })
      continue
    }
  ...
  const lines = [
    ...
    '| --- | --- | --- |',
    ...rows.map((row) => `| \`${row.slider}\` | ${row.changed ? 'sim' : '**não**'} | ${row.note} |`),
    '',
